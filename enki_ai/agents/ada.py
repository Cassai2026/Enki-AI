...
def _get_gesture_controller():
    ...
    if _gesture_controller is None:
        try:
            ...
            _debug_log("[ADA] GestureController initialised.")
        except Exception as exc:
            _debug_log(f"[ADA] GestureController unavailable: {exc}")
...
def _query_forensic_db(keyword: str) -> dict | None:
    ...
    try:
        ...
        _debug_log(f"[ADA] _query_forensic_db error: {exc}")
        return None
    ...
    * ``enki_knowledge.db`` is queried for the nearest Forensic_Evidence entry.
    ...
    try:
        ...
        if gesture == "PINCH":
            ...
            _debug_log("[ADA] [SPATIAL] STRETCH detected — detail_level=HIGH")

        return result

    except Exception as exc:
        _debug_log(f"[ADA] process_spatial_gestures error (kernel-safe): {exc}")
        return None
...
class AudioLoop:
    ...
    def update_permissions(self, new_perms):
        _debug_log(f"[ADA DEBUG] [CONFIG] Updating tool permissions: {new_perms}")
    ...
    def resolve_tool_confirmation(self, request_id, confirmed):
        _debug_log(f"[ADA DEBUG] [RESOLVE] resolve_tool_confirmation called. ID: {request_id}, Confirmed: {confirmed}")
        if request_id in self._pending_confirmations:
            future = self._pending_confirmations[request_id]
            if not future.done():
                _debug_log(f"[ADA DEBUG] [RESOLVE] Future found and pending. Setting result to: {confirmed}")
                future.set_result(confirmed)
            else:
                 _debug_log(f"[ADA DEBUG] [WARN] Request {request_id} future already done. Result: {future.result()}")
        else:
            _debug_log(f"[ADA DEBUG] [WARN] Confirmation Request {request_id} not found in pending dict. Keys: {list(self._pending_confirmations.keys())}")
    ...
    def clear_audio_queue(self):
        ...
        try:
            ...
            while not self.audio_in_queue.empty():
                ...
                count += 1
            if count > 0:
                _debug_log(f"[ADA DEBUG] [AUDIO] Cleared {count} chunks from playback queue due to interruption.")
        except Exception as e:
            _debug_log(f"[ADA DEBUG] [ERR] Failed to clear audio queue: {e}")
    ...
    async def listen_audio(self):
        ...
        if self.audio_source_queue is not None:
            _debug_log("[ADA] Network audio source active (glasses mode). Skipping local microphone.")
            VAD_THRESHOLD = 800
                ...
                try:
                    ...
                    if rms > VAD_THRESHOLD:
                        ...
                        if not self._is_speaking:
                            ...
                            _debug_log(f"[ADA] [VAD/NET] Speech detected (RMS: {rms}). Sending video frame.")
                    ...
                    _debug_log(f"[ADA] Error reading from network audio queue: {e}")
                    await asyncio.sleep(0.1)
        ...
        if self.input_device_name:
            _debug_log(f"[ADA] Attempting to find input device matching: '{self.input_device_name}'")
            ...
            if resolved_input_device_index is not None:
                _debug_log(f"[ADA] Resolved input device '{self.input_device_name}' to index {resolved_input_device_index} ({best_match})")
            else:
                _debug_log(f"[ADA] Could not find device matching '{self.input_device_name}'. Checking index...")
        ...
        if resolved_input_device_index is None and self.input_device_index is not None:
             try:
                 ...
                 _debug_log(f"[ADA] Requesting Input Device Index: {resolved_input_device_index}")
             except ValueError:
                 _debug_log(f"[ADA] Invalid device index '{self.input_device_index}', reverting to default.")
                 resolved_input_device_index = None

        if resolved_input_device_index is None:
             _debug_log("[ADA] Using Default Input Device")
        ...
        try:
            ...
            _debug_log(f"[ADA] [ERR] Failed to open audio input stream: {e}")
            _debug_log("[ADA] [WARN] Audio features will be disabled. Please check microphone permissions.")
            return
            ...
            try:
                ...
                if rms > VAD_THRESHOLD:
                    ...
                    if not self._is_speaking:
                        ...
                        self._is_speaking = True
                        _debug_log(f"[ADA DEBUG] [VAD] Speech Detected (RMS: {rms}). Sending Video Frame.")
                        
                        # Send ONE frame
                        if self._latest_image_payload and self.out_queue:
                            await self.out_queue.put(self._latest_image_payload)
                        else:
                            _debug_log(f"[ADA DEBUG] [VAD] No video frame available to send.")
                    ...
                    if self._is_speaking:
                        if self._silence_start_time is None:
                            ...
                            _debug_log(f"[ADA DEBUG] [VAD] Silence detected. Resetting speech state.")
                            self._is_speaking = False
                            self._silence_start_time = None

            except Exception as e:
                _debug_log(f"Error reading audio: {e}")
                await asyncio.sleep(0.1)

    async def handle_cad_request(self, prompt):
        _debug_log(f"[ADA DEBUG] [CAD] Background Task Started: handle_cad_request('{prompt}')")
        if self.on_cad_status:
        ...
        if self.project_manager.current_project == "temp":
            ...
            _debug_log(f"[ADA DEBUG] [CAD] Auto-creating project: {new_project_name}")
            ...
            if success:
                ...
                try:
                    ...
                    _debug_log(f"[ADA DEBUG] [ERR] Failed to notify auto-project: {e}")
        ...
        if cad_data:
            _debug_log(f"[ADA DEBUG] [OK] CadAgent returned data successfully.")
            _debug_log(f"[ADA DEBUG] [INFO] Data Check: {len(cad_data.get('vertices', []))} vertices, {len(cad_data.get('edges', []))} edges.")
            
            if self.on_cad_data:
                _debug_log(f"[ADA DEBUG] [SEND] Dispatching data to frontend callback...")
                self.on_cad_data(cad_data)
                _debug_log(f"[ADA DEBUG] [SENT] Dispatch complete.")
            ...
            try:
                ...
                _debug_log(f"[ADA DEBUG] [NOTE] Sent completion notification to model.")
            except Exception as e:
                 _debug_log(f"[ADA DEBUG] [ERR] Failed to send completion notification: {e}")

        else:
            _debug_log(f"[ADA DEBUG] [ERR] CadAgent returned None.")
    ...
    async def handle_write_file(self, path, content):
        _debug_log(f"[ADA DEBUG] [FS] Writing file: '{path}'")
        ...
        if self.project_manager.current_project == "temp":
            ...
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            new_project_name = f"Project_{timestamp}"
            _debug_log(f"[ADA DEBUG] [FS] Auto-creating project: {new_project_name}")
        ...
        _debug_log(f"[ADA DEBUG] [FS] Resolved path: '{final_path}'")
        ...
        _debug_log(f"[ADA DEBUG] [FS] Result: {result}")
        try:
             await self.session.send(input=f"System Notification: {result}", end_of_turn=True)
        except Exception as e:
             _debug_log(f"[ADA DEBUG] [ERR] Failed to send fs result: {e}")

    async def handle_read_directory(self, path):
        _debug_log(f"[ADA DEBUG] [FS] Reading directory: '{path}'")
        try:
    ...
    async def handle_read_file(self, path):
        _debug_log(f"[ADA DEBUG] [FS] Reading file: '{path}'")
        try:
            if not os.path.exists(path):
    ...
    async def handle_web_agent_request(self, prompt):
        _debug_log(f"[ADA DEBUG] [WEB] Web Agent Task: '{prompt}'")
        ...
        _debug_log(f"[ADA DEBUG] [WEB] Web Agent Task Returned: {result}")
        ...
        try:
             ...
             _debug_log(f"[ADA DEBUG] [ERR] Failed to send web agent result to model: {e}")
    ...
    async def receive_audio(self):
        ...
        try:
            while True:
                ...
                async for response in turn:
                    ...
                    if response.tool_call:
                        _debug_log("The tool was called")
                        function_responses = []
                        for fc in response.tool_call.function_calls:
                            if fc.name in ["generate_cad", "run_web_agent", "write_file", "read_directory", "read_file", "create_project", "switch_project", "list_projects", "list_smart_devices", "control_light", "discover_printers", "print_stl", "get_print_status", "iterate_cad"]:
                                ...
                                if not confirmation_required:
                                    _debug_log(f"[ADA DEBUG] [TOOL] Permission check: '{fc.name}' -> AUTO-ALLOW")
                                    ...
                                    _debug_log(f"[ADA DEBUG] [STOP] Requesting confirmation for '{fc.name}' (ID: {request_id})")
                                    ...
                                    _debug_log(f"[ADA DEBUG] [CONFIRM] Request {request_id} resolved. Confirmed: {confirmed}")

                                    if not confirmed:
                                        _debug_log(f"[ADA DEBUG] [DENY] Tool call '{fc.name}' denied by user.")
                                ...
                                if fc.name == "generate_cad":
                                    _debug_log(f"\n[ADA DEBUG] --------------------------------------------------")
                                    _debug_log(f"[ADA DEBUG] [TOOL] Tool Call Detected: 'generate_cad'")
                                    _debug_log(f"[ADA DEBUG] [IN] Arguments: prompt='{prompt}'")
                                    ...
                                    _debug_log(f"[ADA DEBUG] [TOOL] Tool Call: 'run_web_agent' with prompt='{prompt}'")
                                    ...
                                    _debug_log(f"[ADA DEBUG] [RESPONSE] Sending function response: {function_response}")
                                    function_responses.append(function_response)
                                    ...
                                    _debug_log(f"[ADA DEBUG] [TOOL] Tool Call: 'write_file' path='{path}'")
                                    ...
                                    path = fc.args["path"]
                                    _debug_log(f"[ADA DEBUG] [TOOL] Tool Call: 'read_directory' path='{path}'")
                                    ...
                                    path = fc.args["path"]
                                    _debug_log(f"[ADA DEBUG] [TOOL] Tool Call: 'read_file' path='{path}'")
                                    ...
                                    name = fc.args["name"]
                                    _debug_log(f"[ADA DEBUG] [TOOL] Tool Call: 'create_project' name='{name}'")
                                    ...
                                    name = fc.args["name"]
                                    _debug_log(f"[ADA DEBUG] [TOOL] Tool Call: 'switch_project' name='{name}'")
                                    ...
                                    if success:
                                        ...
                                        _debug_log(f"[ADA DEBUG] [PROJECT] Sending project context to AI ({len(context)} chars)")
                                        try:
                                            await self.session.send(input=f"System Notification: {msg}\n\n{context}", end_of_turn=False)
                                        except Exception as e:
                                            _debug_log(f"[ADA DEBUG] [ERR] Failed to send project context: {e}")
                                    function_response = types.FunctionResponse(
                                        id=fc.id, name=fc.name, response={"result": msg}
                                    )
                                    function_responses.append(function_response)
                                    ...
                                    _debug_log(f"[ADA DEBUG] [TOOL] Tool Call: 'list_projects'")
                                    ...
                                    _debug_log(f"[ADA DEBUG] [TOOL] Tool Call: 'list_smart_devices'")
                                    ...
                                    _debug_log(f"[ADA DEBUG] [TOOL] Tool Call: 'control_light' Target='{target}' Action='{action}'")
                                    ...
                                    _debug_log(f"[ADA DEBUG] [TOOL] Tool Call: 'discover_printers'")
                                    ...
                                    _debug_log(f"[ADA DEBUG] [TOOL] Tool Call: 'print_stl' STL='{stl_path}' Printer='{printer}'")
                                    ...
                                    printer = fc.args["printer"]
                                    _debug_log(f"[ADA DEBUG] [TOOL] Tool Call: 'get_print_status' Printer='{printer}'")
                                    ...
                                    _debug_log(f"[ADA DEBUG] [TOOL] Tool Call: 'iterate_cad' Prompt='{prompt}'")
                                    ...
                                    if cad_data:
                                        _debug_log(f"[ADA DEBUG] [OK] CadAgent iteration returned data successfully.")
                                        
                                        # Dispatch to frontend
                                        if self.on_cad_data:
                                            _debug_log(f"[ADA DEBUG] [SEND] Dispatching iterated CAD data to frontend...")
                                            self.on_cad_data(cad_data)
                                            _debug_log(f"[ADA DEBUG] [SENT] Dispatch complete.")
                                        ...
                                        _debug_log(f"[ADA DEBUG] [ERR] CadAgent iteration returned None.")
            ...
            _debug_log(f"Error in receive_audio: {e}")
            traceback.print_exc()
    ...
    async def play_audio(self):
        try:
            ...
            import traceback
            _debug_log(f"[ADA] [WARN] Could not open local audio output stream: {e}. "
    ...
    async def run(self, start_message=None):
        ...
        while not self.stop_event.is_set():
            try:
                _debug_log(f"[ADA DEBUG] [CONNECT] Connecting to Gemini Live API...")
                async with (
                    ...
                    if not is_reconnect:
                        if start_message:
                            _debug_log(f"[ADA DEBUG] [INFO] Sending start message: {start_message}")
                        ...
                        _debug_log(f"[ADA DEBUG] [RECONNECT] Connection restored.")
                        # Restore Context
                        _debug_log(f"[ADA DEBUG] [RECONNECT] Fetching recent chat history to restore context...")
                        ...
                        _debug_log(f"[ADA DEBUG] [RECONNECT] Sending restoration context to model...")
                ...
                _debug_log(f"[ADA DEBUG] [STOP] Main loop cancelled.")
                break
                
            except Exception as e:
                # This catches the ExceptionGroup from TaskGroup or direct exceptions
                _debug_log(f"[ADA DEBUG] [ERR] Connection Error: {e}")
                
                if self.stop_event.is_set():
                    break
                
                _debug_log(f"[ADA DEBUG] [RETRY] Reconnecting in {retry_delay} seconds...")
