...
def issue_bounty(mentee_id, task, credits):
    ...
    c.execute(
        ...
        "(id INTEGER PRIMARY KEY, mentee_id TEXT, task TEXT, credits REAL)"
    )
    c.execute(
        "INSERT INTO bounties (mentee_id, task, credits) VALUES (?, ?, ?)",
        (mentee_id, task, credits)
    )
    conn.commit()
...
if __name__ == "__main__":
    issue_bounty("M32_Titan_01", "Weld Carbon-Graphite Frame", 2000.0)
